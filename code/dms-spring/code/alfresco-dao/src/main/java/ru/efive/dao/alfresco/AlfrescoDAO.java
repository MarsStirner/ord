...
public class AlfrescoDAO<T extends AlfrescoNode> implements DAO<T> {
	...
	public AlfrescoDAO(Class<T> class_) throws InitializationException {
		...
		try {
			this.class_ = class_;
			rb = ResourceBundle.getBundle("alfresco-test");
			initialize();
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public boolean createVersion(T data, InputStream inputStream, int authorId, String fileName) {
		...
		try {
			...
			byte[] bytes = ContentUtils.convertToByteArray(inputStream);
			System.out.println("Content length - " + bytes.length);
			getContentService().write(workingCopyReference, Constants.PROP_CONTENT, bytes, new ContentFormat("application/octet-stream", "utf-8"));
			
			Predicate predicate = new Predicate(new Reference[] { workingCopyReference }, null, null);
			
			NamedValue[] comments = new NamedValue[]{Utils.createNamedValue("author", Integer.toString(authorId)), Utils.createNamedValue("revisionFileName", URLEncoder.encode(fileName,"UTF-8"))};
	...
	public boolean createVersion(T data, byte[] bytes, int authorId, String fileName, boolean majorVersion) {
		...
		try {
			...
			NamedValue[] comments = majorVersion?
					new NamedValue[] { Utils.createNamedValue("author", Integer.toString(authorId)), Utils.createNamedValue("revisionFileName", URLEncoder.encode(fileName,"UTF-8")), Utils.createNamedValue("versionType", "MAJOR") }:
					new NamedValue[] { Utils.createNamedValue("author", Integer.toString(authorId)), Utils.createNamedValue("revisionFileName", URLEncoder.encode(fileName,"UTF-8")), Utils.createNamedValue("versionType", "MINOR") };
