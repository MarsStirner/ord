	...
	protected List<IncomingDocument> getHashDocuments(int fromIndex, int toIndex){		
		...
		if(needRefresh){	
			...
			try {			
				...
				this.hashDocuments=new ArrayList<IncomingDocument>(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findAllDocumentsByUser(filters, filter, user,false, false)));				
	...
	protected List<IncomingDocument> getHashDocuments(){		
		...
		if(needRefresh){
			...
			try {			
				...
				result=new ArrayList<IncomingDocument>(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findAllDocumentsByUser(filters, filter, user,false, false)));
