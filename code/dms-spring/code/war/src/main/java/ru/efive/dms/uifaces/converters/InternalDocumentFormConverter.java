...
@FacesConverter("InternalDocumentFormConverter")
public class InternalDocumentFormConverter implements Converter {
	...
	public Object getAsObject(FacesContext context, UIComponent component, String value) {
		...
		try {
			...
			List<DocumentForm> list = sessionManagement.getDictionaryDAO(DocumentFormDAOImpl.class, ApplicationHelper.DOCUMENT_FORM_DAO).findByCategoryAndValue("Внутренние документы",value);
			if (list.size() > 0) {
				result = list.get(0);
			}
			else {
