	...
	public static boolean setOutgoingRegistrationNumber(OutgoingDocument doc) {
		...
		if(in_result.toString().equals("")){
			try {			
				...
				if (document==null){result = false;}else{	
					if (document != null) {	
						if(document.getRegistrationNumber()==null || document.getRegistrationNumber().isEmpty()){
							...
							if(in_nomenclature!=null){
								in_office=sessionManagement.getDAO(RoleDAOHibernate.class, "roleDao").findRoleByType(RoleType.valueOf("OFFICE_"+in_nomenclature.getCategory()));
								in_number.append(in_nomenclature.getCategory()+"-");							
							}else{
								in_office=sessionManagement.getDAO(RoleDAOHibernate.class, "roleDao").findRoleByType(RoleType.valueOf("OFFICE_01"));
								in_number.append("01-");
							}
							...
							if(paperCopies.size()>0){
								for(PaperCopyDocument paperCopy:paperCopies){
									...
									if(paperCopy.getStatusId()<2){paperCopy.setStatusId(2);}
				...
				if (document==null){result = false;}else{	
					if (document != null) {				
						if(document.getRegistrationNumber()==null || document.getRegistrationNumber().isEmpty()){
							...
							StringBuffer in_count=new  StringBuffer("0000"+String.valueOf(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findRegistratedDocumentsByCriteria(in_number.toString())).size()+1));						
							in_number.append(in_count.substring(in_count.length()-4));
							document.setRegistrationNumber(in_number.toString());	
			...
			try {			
				...
				if (document==null){result = false;}else{	
					if (document != null) {	
						if(document.getRegistrationNumber()==null || document.getRegistrationNumber().isEmpty()){				
							Nomenclature in_nomenclature=dictionaryManager.getNomenclatureByUserUNID(document.getSigner().getUNID());
							Role in_administrationRole=sessionManagement.getDAO(RoleDAOHibernate.class, "roleDao").findRoleByType(RoleType.ADMINISTRATOR);
							List<Role> in_roles=new ArrayList<Role>();													
							in_roles.add(in_administrationRole);
			...
			if (template.getRootNodes() != null) {
				for (HumanTaskTreeNode rootNodeTemplate: template.getRootNodeList()) {
					...
					for (HumanTask taskTemplate: rootNodeTemplate.getTaskList()) {
						...
						newTask.setDocumentStatus(DocumentStatus.NEW);
