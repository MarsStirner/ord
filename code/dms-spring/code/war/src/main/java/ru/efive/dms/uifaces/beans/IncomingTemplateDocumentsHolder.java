...
public class IncomingTemplateDocumentsHolder extends AbstractDocumentListHolderBean<IncomingDocument> {	
	...
	protected List<IncomingDocument> getHashDocuments(int fromIndex, int toIndex){		
		...
		if(needRefresh){			
			try {			
				this.hashDocuments=new ArrayList<IncomingDocument>(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findAllDocuments(filters,filter, false, true)));				
	...
	protected List<IncomingDocument> getHashDocuments(){		
		...
		if(needRefresh){		
			try {										
				result=new ArrayList<IncomingDocument>(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findAllDocuments(filters,filter, false, true)));				
