...
public class NumeratorsHolder extends AbstractDocumentListHolderBean<Numerator> {
	...
	protected List<Numerator> getHashDocuments(int fromIndex, int toIndex){		
		...
		if(needRefresh){			
			try {							
				this.hashDocuments=new ArrayList<Numerator>(new HashSet<Numerator>(sessionManagement.getDAO(NumeratorDAOImpl.class, "numeratorDao").findAllDocuments(filters,filter,false, true)));				
	...
	protected List<Numerator> getHashDocuments(){		
		...
		if(needRefresh){
			try {			
				result=new ArrayList<Numerator>(new HashSet<Numerator>(sessionManagement.getDAO(NumeratorDAOImpl.class, "numeratorDao").findAllDocuments(filters,filter,false, true)));				
