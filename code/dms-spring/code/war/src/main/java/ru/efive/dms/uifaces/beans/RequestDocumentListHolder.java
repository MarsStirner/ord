...
public class RequestDocumentListHolder extends AbstractDocumentListHolderBean<RequestDocument> {
	...
	protected List<RequestDocument> getHashDocuments(int fromIndex, int toIndex){		
		...
		if(needRefresh){			
			try {			
				...
				this.hashDocuments=new ArrayList<RequestDocument>(new HashSet<RequestDocument>(sessionManagement.getDAO(RequestDocumentDAOImpl.class, ApplicationHelper.REQUEST_DOCUMENT_FORM_DAO).findAllDocumentsByUser(filter, user,false, false)));
	...
	protected int getTotalCount() {
		...
		try {
			User user=sessionManagement.getLoggedUser(); 
			user = sessionManagement.getDAO(UserDAOHibernate.class, ApplicationHelper.USER_DAO).findByLoginAndPassword(user.getLogin(),user.getPassword());
			return new Long(sessionManagement.getDAO(RequestDocumentDAOImpl.class, ApplicationHelper.REQUEST_DOCUMENT_FORM_DAO).countAllDocumentsByUser(filter,
	...
	protected List<RequestDocument> loadDocuments() {
		...
		try {
			User user=sessionManagement.getLoggedUser(); 
			user = sessionManagement.getDAO(UserDAOHibernate.class, ApplicationHelper.USER_DAO).findByLoginAndPassword(user.getLogin(),user.getPassword());
			result = sessionManagement.getDAO(RequestDocumentDAOImpl.class, ApplicationHelper.REQUEST_DOCUMENT_FORM_DAO).findAllDocumentsByUser(filter, user,
