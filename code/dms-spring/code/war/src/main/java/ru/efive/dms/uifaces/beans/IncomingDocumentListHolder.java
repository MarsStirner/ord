	...
	protected List<IncomingDocument> getHashDocuments(int fromIndex, int toIndex){
		...
		if(needRefresh){
			...
			try {
				...
				this.hashDocuments=new ArrayList<IncomingDocument>(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findAllDocumentsByUser(filter, user,false, false)));
	...
	protected List<IncomingDocument> getHashDocuments(){
		...
		if(needRefresh){
			...
			try {
				User user=sessionManagement.getLoggedUser();
				user=sessionManagement.getDAO(UserDAOHibernate.class, ApplicationHelper.USER_DAO).findByLoginAndPassword(user.getLogin(),user.getPassword());
				result=new ArrayList<IncomingDocument>(new HashSet<IncomingDocument>(sessionManagement.getDAO(IncomingDocumentDAOImpl.class, "incomingDao").findAllDocumentsByUser(filter, user,false, false)));
