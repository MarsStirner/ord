...
public class RouteTemplateListBean extends AbstractDocumentListHolderBean<RouteTemplate> {
	...
	protected int getTotalCount() {
		int result = 0;
		try {
			result = new Long(sessionManagement.getDAO(EngineDAOImpl.class, "engineDao").countRouteTemplates(sessionManagement.getLoggedUser(), false)).intValue();
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected List<RouteTemplate> loadDocuments() {
		...
		try {
			result = sessionManagement.getDAO(EngineDAOImpl.class, "engineDao").findRouteTemplates(sessionManagement.getLoggedUser(), false,
